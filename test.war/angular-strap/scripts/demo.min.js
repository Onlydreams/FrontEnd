...
angular.module("mgcrea.ngStrapDocs").config(["$modalProvider", function (e) {
		...
		$scope.modal = {
			title : "Title",
			content : "Hello Modal<br />This is a multiline mmmmmmmmmmm message!"
		}
	}
]),

angular.module("mgcrea.ngStrapDocs").config(["$asideProvider", function (e) {
		angular.extend(e.defaults, {
			container : "body",
			html : !0
		})
	}
]).controller("AsideDemoCtrl", ["$scope", function (e) {
		e.aside = {
			title : "Title",
...
angular.module("mgcrea.ngStrapDocs").controller("AlertDemoCtrl", ["$scope", "$templateCache", "$timeout", "$alert", function (e, a, t, o) {
		e.alert = {
		...
		e.showAlert = function () {
			l.show()
		}
	}
]), 

angular.module("mgcrea.ngStrapDocs").controller("ButtonDemoCtrl", ["$scope", function (e) {
		e.button = {
			toggle : !1,
			checkbox : {
				left : !1,
...
angular.module("mgcrea.ngStrapDocs").controller("SelectDemoCtrl", ["$scope", "$http", function (e) {
		e.selectedIcon = "",
		e.selectedIcons = ["Globe", "Heart"],
		e.icons = [{
				value : "Gear",
		...
		e.selectedMonth = 0,
		e.months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
	}
]),

angular.module("mgcrea.ngStrapDocs").config(["$datepickerProvider", function (e) {
		angular.extend(e.defaults, {
			dateFormat : "dd/MM/yyyy",
			startWeek : 1
		})
	}
]).controller("DatepickerDemoCtrl", ["$scope", "$http", function (e) {
		e.selectedDate = new Date,
		e.selectedDateAsNumber = Date.UTC(1986, 1, 22),
		e.getType = function (a) {
			return Object.prototype.toString.call(e[a])
		},
		e.clearDates = function () {
			e.selectedDate = null
		}
	}
]), 

angular.module("mgcrea.ngStrapDocs").controller("TimepickerDemoCtrl", ["$scope", "$http", function (e) {
		e.time = new Date(1970, 0, 1, 10, 30),
		e.selectedTimeAsNumber = 36e6,
		e.selectedTimeAsString = "10:00",
		e.sharedDate = new Date((new Date).setMinutes(0))
	}
]),
...
angular.module("mgcrea.ngStrapDocs").config(["$popoverProvider", function (e) {
		angular.extend(e.defaults, {
			html : !0
		})
	}
]).controller("PopoverDemoCtrl", ["$scope", function (e) {
		e.popover = {
			title : "Title",
...
angular.module("mgcrea.ngStrapDocs").config(["$dropdownProvider", function (e) {
		angular.extend(e.defaults, {
			html : !0
		})
	}
]).controller("DropdownDemoCtrl", ["$scope", "$alert", function (e, a) {
		e.dropdown = [{
...
angular.module("mgcrea.ngStrapDocs").controller("NavbarDemoCtrl", ["$scope", "$location", function (e, a) {
		e.$location = a
	}
]), angular.module("mgcrea.ngStrapDocs").controller("TypeaheadDemoCtrl", ["$scope", "$templateCache", "$http", function (e, a, t) {
		e.selectedState = "",
		...
		e.getAddress = function (e) {
			...
			return t.get("http://maps.googleapis.com/maps/api/geocode/json", {
				params : a
			}).then(function (e) {
...
angular.module("mgcrea.ngStrapDocs").controller("TabDemoCtrl", ["$scope", "$templateCache", function (e) {
		e.tabs = [{
				title : "Home",
...
angular.module("mgcrea.ngStrapDocs").controller("CollapseDemoCtrl", ["$scope", "$templateCache", function (e) {
		e.panels = [{
		...
		e.panels.activePanel = 1,
		e.pushPanel = function () {
			e.panels.push({
